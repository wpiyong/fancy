...
using gColorFancy.Model;
using gColorFancy.View;
...
namespace gColorFancy.ViewModel
{
    ...
    class ControlViewModel : ViewModelBase
    {
        ...
        ImageAnalyzer imgAnalyzer = new ImageAnalyzer_FancyColor();
        ...
        void Measure(bool many)
        {
            ...
            var dialogControlNumberWindow = new gColorFancy.View.ControlNumber();
            var dialogViewModel = new gColorFancy.ViewModel.ControlNumberViewModel();
        ...
        void Measure_doWork(object sender, DoWorkEventArgs e)
        {
            ...
            if (cameraControl.GetImages(cameraControl.cameras[(int)CameraType.Top], out img, out imgSrc))
            {
                ...
                if (!imgAnalyzer.check_diamond_centered(ref img, (int)(img.Width / 2), (int)(img.Height / 2),
                    ref comment, App.Settings.MaxCenterDistanceDiff))
                {
                    App.LogEntry.AddEntry("Error: " + comment);
                    return;
                }
            }
            else
            ...
            if (backgroundImageList.Count >= imageList.Count)
            {
                ...
                if (usingCrop)
                {
                    ...
                    if (UsingManualMask)
                    {
                        System.Windows.Application.Current.Dispatcher.Invoke(() =>
                        {
                            bmpMask.Save(@"C:\gColorFancy\manualmask.bmp");
                        });
